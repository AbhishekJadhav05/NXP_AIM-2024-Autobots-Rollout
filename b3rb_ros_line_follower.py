...
class LineFollower(Node):
    ...
    def __init__(self):
        ...
        self.obs, self.collided = 0.0, False
    ...
    def edge_vectors_callback(self, message):
        ...
        if (vectors.vector_count == 1):  # curve.
            ...
            speed = SPEED_25_PERCENT * (np.abs(math.cos(p_turn))**(1/2))
        ...
        if (vectors.vector_count == 2):  # straight.
            ...
            p_turn = deviation / half_width
            speed = speed * (np.abs(math.cos(p_turn))**(3/5))
            #speed = SPEED_MAX
        ...
        if self.obstacle_detected is True:
            ...
            speed = min(0.4,0.7*(np.abs(math.cos(p_turn))**(2/3)))
        
        if self.collided is True:
            turn = self.prevTurn*1.7 
            speed = -0.5
        ...
        if self.prevSpeed < 0 and speed>0:
            speed = self.prevSpeed + 0.2*speed
